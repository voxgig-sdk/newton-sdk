...
class NewtonRetryFeature extends NewtonBaseFeature
{
    ...
    public function init(NewtonContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NewtonContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NewtonContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NewtonContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
