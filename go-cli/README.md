...
boru-driven command-line client **and** interactive REPL for the Newton
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/newton-cli)
make build
...
./newton-cli load 1 abs            # {id:1} shorthand
./newton-cli load '{id:1}' abs       # explicit match map

# 5. Override the API base URL for a single call
NEWTON_BASE=https://api.example.com ./newton-cli load 1 abs
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/newton-cli
   ```
...
```sh
./newton-cli load 1 abs          # scalar shorthand for {id:1}
./newton-cli load '{id:1}' abs     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `newton>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `abs`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/newton-cli`. |
...
(`./newton-cli <expr>`) and interactively (the REPL), and expressions compose the
