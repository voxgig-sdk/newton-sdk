// Command Newton-cli is a CLI + REPL for the Newton SDK,
...
const entitiesHelp = "abs arcco arcsin arctan area cos derive factor integrate log simplify sin tan tangent zero"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NEWTON_APIKEY (api key), NEWTON_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NEWTON_APIKEY carries the API key and
	// NEWTON_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NEWTON_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NEWTON_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNewtonSDK(opts)
