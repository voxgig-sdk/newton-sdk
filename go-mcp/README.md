...
[MCP](https://modelcontextprotocol.io) server exposing the Newton SDK as
two agent tools — `newton_list` and `newton_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/newton-mcp)
make build

# 2. Provide credentials via the environment
export NEWTON_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user newton \
  -- /absolute/path/to/newton-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./newton-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// newton_list: first page of records
{ "entity": "abs" }
{ "entity": "abs", "query": { } }

// newton_load: one record by id
{ "entity": "abs", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/newton-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user newton \
     -- "$PWD"/dist/*/newton-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `newton_list` and `newton_load` tools now appear
   in new sessions. Ask the agent to *"list abs using newton"*
   and it calls `newton_list` with `{"entity":"abs"}`.
...
```sh
export NEWTON_APIKEY=sk_live_xxx            # API key
export NEWTON_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `newton_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `newton_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NEWTON_APIKEY` | API key sent with every request. |
| `NEWTON_BASE` | Optional override of the API base URL. |

### Entities

The 15 entities valid as the `entity` argument:

abs | arcco | arcsin | arctan | area | cos | derive | factor | integrate | log | simplify | sin | tan | tangent | zero

### Smoke test via HTTP (raw JSON-RPC)

```sh
./newton-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"newton_load","arguments":{"entity":"abs","query":{"id":1}}}}'
```
