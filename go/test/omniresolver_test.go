...
import (
	...
	omni "github.com/voxgig-sdk/newton-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NewtonError); is {
				out := map[string]any{
					"name":    "NewtonError",
...
func omniCtx(arg any, client *sdk.NewtonSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NewtonSDK); is {
			client = live
