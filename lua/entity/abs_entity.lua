...
local AbsEntity = {}
AbsEntity.__index = AbsEntity


function AbsEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AbsEntity)
  self._name = "abs"
  self._client = client
...
function AbsEntity:get_name()
  return self._name
end


function AbsEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AbsEntity.new(self._client, opts)
end
...
function AbsEntity:mark_deleted()
  self._deleted = true
end


function AbsEntity:deleted()
  return true == self._deleted
end


function AbsEntity:data_set(args)
  if args ~= nil then
...
function AbsEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AbsEntity:match_set(args)
  if args ~= nil then
...
function AbsEntity:stream(action, args, callopts)
...
function AbsEntity:_run_op(ctx, post_done)
  local utility = self._utility
