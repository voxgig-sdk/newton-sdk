...
local ArccoEntity = {}
ArccoEntity.__index = ArccoEntity


function ArccoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArccoEntity)
  self._name = "arcco"
  self._client = client
...
function ArccoEntity:get_name()
  return self._name
end


function ArccoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArccoEntity.new(self._client, opts)
end
...
function ArccoEntity:mark_deleted()
  self._deleted = true
end


function ArccoEntity:deleted()
  return true == self._deleted
end


function ArccoEntity:data_set(args)
  if args ~= nil then
...
function ArccoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArccoEntity:match_set(args)
  if args ~= nil then
...
function ArccoEntity:match_get()
...
function ArccoEntity:stream(action, args, callopts)
...
function ArccoEntity:_run_op(ctx, post_done)
  local utility = self._utility
