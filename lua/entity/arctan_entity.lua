...
local ArctanEntity = {}
ArctanEntity.__index = ArctanEntity


function ArctanEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArctanEntity)
  self._name = "arctan"
  self._client = client
...
function ArctanEntity:get_name()
  return self._name
end


function ArctanEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArctanEntity.new(self._client, opts)
end
...
function ArctanEntity:mark_deleted()
  self._deleted = true
end


function ArctanEntity:deleted()
  return true == self._deleted
end


function ArctanEntity:data_set(args)
  if args ~= nil then
...
function ArctanEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArctanEntity:match_set(args)
  if args ~= nil then
...
function ArctanEntity:match_get()
...
function ArctanEntity:stream(action, args, callopts)
...
---@param reqmatch ArctanLoadMatch
---@param ctrl? table
---@return Arctan
---@return string? err
function ArctanEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ArctanEntity:_run_op(ctx, post_done)
  local utility = self._utility
