...
local CosEntity = {}
CosEntity.__index = CosEntity


function CosEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CosEntity)
  self._name = "cos"
  self._client = client
...
function CosEntity:get_name()
  return self._name
end


function CosEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CosEntity.new(self._client, opts)
end
...
function CosEntity:mark_deleted()
  self._deleted = true
end


function CosEntity:deleted()
  return true == self._deleted
end


function CosEntity:data_set(args)
  if args ~= nil then
...
function CosEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CosEntity:match_set(args)
  if args ~= nil then
...
function CosEntity:stream(action, args, callopts)
...
---@param reqmatch CosLoadMatch
---@param ctrl? table
---@return Cos
---@return string? err
function CosEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CosEntity:_run_op(ctx, post_done)
  local utility = self._utility
