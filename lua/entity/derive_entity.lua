...
local DeriveEntity = {}
DeriveEntity.__index = DeriveEntity


function DeriveEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DeriveEntity)
  self._name = "derive"
  self._client = client
...
function DeriveEntity:get_name()
  return self._name
end


function DeriveEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DeriveEntity.new(self._client, opts)
end
...
function DeriveEntity:mark_deleted()
  self._deleted = true
end


function DeriveEntity:deleted()
  return true == self._deleted
end


function DeriveEntity:data_set(args)
  if args ~= nil then
...
function DeriveEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DeriveEntity:match_set(args)
  if args ~= nil then
...
function DeriveEntity:match_get()
...
function DeriveEntity:stream(action, args, callopts)
...
---@param reqmatch DeriveLoadMatch
---@param ctrl? table
---@return Derive
---@return string? err
function DeriveEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DeriveEntity:_run_op(ctx, post_done)
  local utility = self._utility
