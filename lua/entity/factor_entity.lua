...
local FactorEntity = {}
FactorEntity.__index = FactorEntity


function FactorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FactorEntity)
  self._name = "factor"
  self._client = client
...
function FactorEntity:get_name()
  return self._name
end


function FactorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FactorEntity.new(self._client, opts)
end
...
function FactorEntity:mark_deleted()
  self._deleted = true
end


function FactorEntity:deleted()
  return true == self._deleted
end


function FactorEntity:data_set(args)
  if args ~= nil then
...
function FactorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FactorEntity:match_set(args)
  if args ~= nil then
...
function FactorEntity:match_get()
...
function FactorEntity:stream(action, args, callopts)
...
function FactorEntity:_run_op(ctx, post_done)
  local utility = self._utility
