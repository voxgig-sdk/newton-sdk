...
local IntegrateEntity = {}
IntegrateEntity.__index = IntegrateEntity


function IntegrateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IntegrateEntity)
  self._name = "integrate"
  self._client = client
...
function IntegrateEntity:get_name()
  return self._name
end


function IntegrateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IntegrateEntity.new(self._client, opts)
end
...
function IntegrateEntity:mark_deleted()
  self._deleted = true
end


function IntegrateEntity:deleted()
  return true == self._deleted
end


function IntegrateEntity:data_set(args)
  if args ~= nil then
...
function IntegrateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IntegrateEntity:match_set(args)
  if args ~= nil then
...
function IntegrateEntity:match_get()
...
function IntegrateEntity:stream(action, args, callopts)
...
function IntegrateEntity:_run_op(ctx, post_done)
  local utility = self._utility
