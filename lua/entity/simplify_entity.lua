...
local SimplifyEntity = {}
SimplifyEntity.__index = SimplifyEntity


function SimplifyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SimplifyEntity)
  self._name = "simplify"
  self._client = client
...
function SimplifyEntity:get_name()
  return self._name
end


function SimplifyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SimplifyEntity.new(self._client, opts)
end
...
function SimplifyEntity:mark_deleted()
  self._deleted = true
end


function SimplifyEntity:deleted()
  return true == self._deleted
end


function SimplifyEntity:data_set(args)
  if args ~= nil then
...
function SimplifyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SimplifyEntity:match_set(args)
  if args ~= nil then
...
function SimplifyEntity:match_get()
...
function SimplifyEntity:stream(action, args, callopts)
...
function SimplifyEntity:_run_op(ctx, post_done)
  local utility = self._utility
