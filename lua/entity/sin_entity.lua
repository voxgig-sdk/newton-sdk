...
local SinEntity = {}
SinEntity.__index = SinEntity


function SinEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SinEntity)
  self._name = "sin"
  self._client = client
...
function SinEntity:get_name()
  return self._name
end


function SinEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SinEntity.new(self._client, opts)
end
...
function SinEntity:mark_deleted()
  self._deleted = true
end


function SinEntity:deleted()
  return true == self._deleted
end


function SinEntity:data_set(args)
  if args ~= nil then
...
function SinEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SinEntity:match_set(args)
  if args ~= nil then
...
function SinEntity:stream(action, args, callopts)
...
function SinEntity:_run_op(ctx, post_done)
  local utility = self._utility
