...
local TanEntity = {}
TanEntity.__index = TanEntity


function TanEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TanEntity)
  self._name = "tan"
  self._client = client
...
function TanEntity:get_name()
  return self._name
end


function TanEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TanEntity.new(self._client, opts)
end
...
function TanEntity:mark_deleted()
  self._deleted = true
end


function TanEntity:deleted()
  return true == self._deleted
end


function TanEntity:data_set(args)
  if args ~= nil then
...
function TanEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TanEntity:match_set(args)
  if args ~= nil then
...
function TanEntity:stream(action, args, callopts)
...
---@param reqmatch TanLoadMatch
---@param ctrl? table
---@return Tan
---@return string? err
function TanEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function TanEntity:_run_op(ctx, post_done)
  local utility = self._utility
