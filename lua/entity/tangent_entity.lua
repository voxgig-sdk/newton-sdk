...
local TangentEntity = {}
TangentEntity.__index = TangentEntity


function TangentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TangentEntity)
  self._name = "tangent"
  self._client = client
...
function TangentEntity:get_name()
  return self._name
end


function TangentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TangentEntity.new(self._client, opts)
end
...
function TangentEntity:mark_deleted()
  self._deleted = true
end


function TangentEntity:deleted()
  return true == self._deleted
end


function TangentEntity:data_set(args)
  if args ~= nil then
...
function TangentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TangentEntity:match_set(args)
  if args ~= nil then
...
function TangentEntity:match_get()
...
function TangentEntity:stream(action, args, callopts)
...
---@param reqmatch TangentLoadMatch
---@param ctrl? table
---@return Tangent
---@return string? err
function TangentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function TangentEntity:_run_op(ctx, post_done)
  local utility = self._utility
