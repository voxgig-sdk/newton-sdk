...
local ZeroEntity = {}
ZeroEntity.__index = ZeroEntity


function ZeroEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ZeroEntity)
  self._name = "zero"
  self._client = client
...
function ZeroEntity:get_name()
  return self._name
end


function ZeroEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ZeroEntity.new(self._client, opts)
end
...
function ZeroEntity:mark_deleted()
  self._deleted = true
end


function ZeroEntity:deleted()
  return true == self._deleted
end


function ZeroEntity:data_set(args)
  if args ~= nil then
...
function ZeroEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ZeroEntity:match_set(args)
  if args ~= nil then
...
function ZeroEntity:stream(action, args, callopts)
...
---@param reqmatch ZeroLoadMatch
---@param ctrl? table
---@return Zero
---@return string? err
function ZeroEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ZeroEntity:_run_op(ctx, post_done)
  local utility = self._utility
