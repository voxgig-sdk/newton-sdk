...
class IntegrateEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new IntegrateEntity($this->_client, $opts);
    }

    /**
     * @param Integrate|array $args Integrate data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NewtonHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Integrate|array The current Integrate data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Integrate fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NewtonHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Integrate fields).
     */
     ...
     *
     * @param IntegrateLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed IntegrateLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Integrate|array The loaded Integrate as an assoc-array at the
     *   SDK boundary; throws NewtonError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = NewtonHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
