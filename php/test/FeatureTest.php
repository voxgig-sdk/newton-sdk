...
class FtCtrl extends NewtonControl
{
...
class FtClient
{
    ...
    private NewtonUtility $utility;

    public function __construct(NewtonUtility $utility, array $options)
    {
    ...
    public function get_utility(): NewtonUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NewtonContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NewtonContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NewtonConfig::make_feature($name);
    ...
    private static function buildUrl(NewtonSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NewtonError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NewtonResponse($fetched);
        }
    ...
    private function fail(NewtonContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NewtonContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
