...
//    only - a typed NewtonContext there would make every ctx
...
final class NewtonOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new NewtonContext($clean, null);
    ...
    public static function err_from_map($m): ?NewtonError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new NewtonError(is_string($code) ? $code : '', $msg);
    }
