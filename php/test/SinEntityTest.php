...
class SinEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = NewtonSDK::test(null, null);
        $ent = $testsdk->Sin(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = sin_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "sin." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NEWTON_TEST_SIN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $sin_ref01_match_dt0 = [];
        [$sin_ref01_data_dt0_loaded, $err] = $sin_ref01_ent->load($sin_ref01_match_dt0, null);
        $this->assertNull($err);
...
function sin_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/sin/SinTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["sin01", "sin02", "sin03"] as $k) {
    ...
    $entid_env_raw = getenv("NEWTON_TEST_SIN_ENTID");
