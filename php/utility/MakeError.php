...
class NewtonMakeError
{
    public static function call(?NewtonContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new NewtonContext([], null);
        }
        $op = $ctx->op ?? new NewtonOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new NewtonResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof NewtonError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new NewtonError('', $msg, $ctx);
