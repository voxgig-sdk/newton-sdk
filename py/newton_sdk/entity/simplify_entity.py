...
class SimplifyEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SimplifyEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Simplify:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Simplify().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
