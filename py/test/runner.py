...
class NewtonTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NewtonTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NewtonTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NewtonTestRunner.getenv("NEWTON_TEST_LIVE")
        override = NewtonTestRunner.getenv("NEWTON_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NewtonTestRunner.getenv(key)
        ...
        explain = NewtonTestRunner.getenv("NEWTON_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NEWTON_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NewtonTestRunner._test_control is not None:
            return NewtonTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NewtonTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NewtonTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NewtonTestRunner.load_test_control()
...
def load_env_local():
    NewtonTestRunner.load_env_local()


def env_override(m):
    return NewtonTestRunner.env_override(m)


def entity_data(v):
    return NewtonTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NewtonTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NewtonTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NewtonTestRunner.load_test_control()


def live_client_options():
    return NewtonTestRunner.live_client_options()


def live_delay_ms():
    return NewtonTestRunner.live_delay_ms()
