...
class TestSinEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Sin(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _sin_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "sin." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NEWTON_TEST_SIN_ENTID JSON to run live")
        client = setup["client"]
        ...
        sin_ref01_data = None
        if len(sin_ref01_data_raw) > 0:
            sin_ref01_data = helpers.to_map(sin_ref01_data_raw[0][1])

        # LOAD
        sin_ref01_ent = client.Sin(None)
        sin_ref01_match_dt0 = {}
        sin_ref01_data_dt0_loaded, err = sin_ref01_ent.load(sin_ref01_match_dt0, None)
        assert err is None
        assert sin_ref01_data_dt0_loaded is not None



def _sin_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/sin/SinTestData.json")
