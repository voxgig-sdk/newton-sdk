...
class FactorEntity
  ...
  def make
    opts = @_entopts.dup
    FactorEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NewtonHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Factor, Hash] the current Factor data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NewtonHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Factor fields)
  def match_get
  ...
  #
  # @param reqmatch [FactorLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Factor.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Factor, Hash] the loaded Factor; raises NewtonError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NewtonHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
