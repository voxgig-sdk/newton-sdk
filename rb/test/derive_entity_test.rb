...
class DeriveEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = derive_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "derive." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NEWTON_TEST_DERIVE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    derive_ref01_data = nil
    if derive_ref01_data_raw.length > 0
    ...
    derive_ref01_data_dt0_loaded, err = derive_ref01_ent.load(derive_ref01_match_dt0, nil)
    assert_nil err
...
def derive_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "derive", "DeriveTestData.json")
  ...
  entid_env_raw = ENV["NEWTON_TEST_DERIVE_ENTID"]
