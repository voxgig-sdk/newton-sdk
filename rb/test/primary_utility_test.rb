...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = NewtonOmni.make_runner(TEST_JSON_FILE, NewtonSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : NewtonOmni.errify(err)
    end
