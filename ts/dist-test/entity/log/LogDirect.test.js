...
(0, node_test_1.describe)('LogDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-log', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-log', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['NEWTON_TEST_LOG_ENTID'];
