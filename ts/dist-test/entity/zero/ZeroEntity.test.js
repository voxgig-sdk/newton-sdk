...
(0, node_test_1.describe)('ZeroEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'zero.' + op, live))
                return;
        }
        ...
        let zero_ref01_data = Object.values(setup.data.existing.zero)[0];
        // LOAD
        const zero_ref01_ent = client.Zero();
        const zero_ref01_match_dt0 = {};
        zero_ref01_match_dt0.id = zero_ref01_data.id;
        const zero_ref01_data_dt0 = (await zero_ref01_ent.load(zero_ref01_match_dt0)).data();
        (0, node_assert_1.default)(zero_ref01_data_dt0.id === zero_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/zero/ZeroTestData.json');
    ...
    let idmap = transform(['zero01', 'zero02', 'zero03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['NEWTON_TEST_ZERO_ENTID'];
...
//# sourceMappingURL=ZeroEntity.test.js.map